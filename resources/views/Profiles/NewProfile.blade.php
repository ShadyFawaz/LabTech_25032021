...
<html>   
<head>    
<title> LabTech Software (New Profile) </title>  
<style>   
Body {  
  font-family: serif;  
...
@endif
<form name="NewProfile"  onsubmit="return validateForm()" action = "{{url('newprofilecreate')}}" method = "post">
    {{ method_field('post') }}
    {{ csrf_field() }}

    <center> <h1> {{__('title.create_new_profile')}} </h1> </center>        
            <div >
            <b><label style="color:red">{{__('title.profile_new')}}  </label></b>   
            <input type="text"  name="profile_name" required>
            </div>
            <div>
            <b><label> {{__('title.active_profile_new')}} </label> </b>  
