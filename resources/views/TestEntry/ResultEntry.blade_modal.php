...
@include('MainMenu')
<body>
<center> <h1   style="background-color:DodgerBlue;"> Result Entry </h1> </center>   
<center> <b    style="background-color:lightblue;"> {{ $users[0]->subgroup_name }} </b> </center>   
         <form action = "{{url('editresultentry/'.($users[0]->regkey),($users[0]->subgroup))}}" method = "post">
    {{ method_field('post') }}
    {{ csrf_field() }}
...
<tr>
<td>{{ $users[0]->visit_no }}</td>
<td>{{ $users[0]->patient_name }}</td>
<td>{{Carbon\Carbon::parse($users[0]->req_date)->format('Y-m-d g:i A')}} </td>
<td>{{ $users[0]->gender }} </td>
<td>{{ $users[0]->ag }} / {{ $users[0]->age }}</td>
</tr>
</table>
</center>
    <td><input type="submit" value="Save All"/> </td>
    <td><a href = "{{url('verifyresultentry/'.($users[0]->regkey),($users[0]->subgroup))}}" class="btn btn-xs btn-info pull-right">Verify</a></td>
    <td><a href = "{{url('report/'.($users[0]->regkey),($users[0]->subgroup))}}" target="_blank">Preview Result</a></td>
    <!-- <td><a href = '' data-toggle="modal" class="ls-modal" data-target="#rt-modal-{{$users[0]->regkey}}-{{$users[0]->subgroup}}" data-remote="/report/{{$users[0]->regkey}}/{{$users[0]->subgroup}}">Preview Modal</a></td> -->
...
<!-- <td><a tabindex="-1" href = "/resulttracking/{{$user->result_id}}" target="_blank">RT</a></td> -->
