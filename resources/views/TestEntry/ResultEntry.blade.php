...
@if($user->verified)
@if($user->low && $user->high && $user->result)
@if ($user->result > $user->high )
@if(Auth::user()->hasPermissionTo(107))
<td><input style="background-color:red;" class="results" id="result-{{$user->result_id}}" onchange='check({{$user->result_id}})' style="width:200px;font-size:12px"  type="string" name="result[{{$user->result_id}}]" value="{{ $user->result }}" {{ $user->TestData->calculated? "readonly":"" }}></td>
@else
<td><input style="background-color:red;" readonly="readonly" class="results" id="result-{{$user->result_id}}" onchange='check({{$user->result_id}})' style="width:200px;font-size:12px"  type="string" name="result[{{$user->result_id}}]" value="{{ $user->result }}" {{ $user->TestData->calculated? "readonly":"" }}></td>
@endif
@elseif ($user->result  < $user->low && $user->result !== '.')
@if(Auth::user()->hasPermissionTo(107))
<td><input style="background-color:pink;" class="results" id="result-{{$user->result_id}}" onchange='check({{$user->result_id}})' style="width:200px;font-size:12px"  type="string" name="result[{{$user->result_id}}]" value="{{ $user->result }}" {{ $user->TestData->calculated? "readonly":"" }}></td>
@else
<td><input style="background-color:pink;" readonly="readonly" class="results" id="result-{{$user->result_id}}" onchange='check({{$user->result_id}})' style="width:200px;font-size:12px"  type="string" name="result[{{$user->result_id}}]" value="{{ $user->result }}" {{ $user->TestData->calculated? "readonly":"" }}></td>
@endif
...
@elseif ($user->result  <= $user->high && $user->result  >= $user->low ) 
@if(Auth::user()->hasPermissionTo(107))
<td><input class="results" id="result-{{$user->result_id}}" onchange='check({{$user->result_id}})' style="width:200px;font-size:12px"  type="string" name="result[{{$user->result_id}}]" value="{{ $user->result }}" {{ $user->TestData->calculated? "readonly":"" }}></td>
@else
<td><input readonly="readonly" class="results" id="result-{{$user->result_id}}" onchange='check({{$user->result_id}})' style="width:200px;font-size:12px"  type="string" name="result[{{$user->result_id}}]" value="{{ $user->result }}" {{ $user->TestData->calculated? "readonly":"" }}></td>
...
<!-- <td><a tabindex="-1" href = "{{url('resulttracking/'.$user->result_id)}}" target="_blank">RT</a></td> -->
<td><input type="button" tabindex="-1" onclick="resultTracking({{$user->result_id}})" value="RT"></input></td>
...
<!-- <div class="modal " data-remote="{{url('resulttracking/'.($user->result_id))}}" id="rt-modal-{{$user->test_id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
// });
$('#rt-modal-{{$user->test_id}}').on('show.bs.modal', function (e) {
  console.log(this);
...
function resultTracking(result_id){

  var page     = "{{url('resulttracking/')}}/" + result_id;
