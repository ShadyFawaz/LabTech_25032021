...
<title> LabTech Software (New Profile Test)</title>  
<style>   
Body {  
  font-family:  serif;  
...
@endif
<form role="form" name="NewProfileTest"  onsubmit="return validateForm()" action = "{{url('newprofiletestcreate/'.$profile_id)}}" method = "post">
    {{ method_field('post') }}
    {{ csrf_field() }}
	    
    <center> <h1 > {{__('title.create_new_profile_test')}} </h1> </center>   
        <div >   
        <b><label>{{__('title.profile_profile_test_new')}} </label>   </b>
            <input hidden readonly="readonly" type = 'integer' name = 'profile_id'
                value = '{{$profile_id}}'/>{{$profile_name}} </td>
            </div>
