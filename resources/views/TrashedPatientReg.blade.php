...
<html>
<head>
<title>LabTech Software (Delete Patient)</title>
<style>   
Body {  
  font-family:  serif;  
...
<form action = "{{url('/testreg/'.($users[0]->regkey))}}" method = "get">
@endif
    {{ method_field('post') }}
    {{ csrf_field() }}
...
<td><input type="button" onclick="deletetest({{$user->testreg_id}})" value="{{__('title.delete_testreg_btn')}}" ></input></td>
@endif
@endif
@endif
...
@if(isset(Auth::user()->login_name))   
@if(Auth::user()->hasPermissionTo(119))
<td><input type="button" onclick="restoretest({{$user->testreg_id}})" value="{{__('title.restore_testreg_btn')}}" ></input></td>
@endif
@endif
@endif
</tr>
...
<a><input type="button" onclick="reset()"  value="{{__('title.reset_regtests')}}" ></input></a>
...
function reset(){
  var page     = "{{url('patientreset',$users[0]->regkey)}}";
