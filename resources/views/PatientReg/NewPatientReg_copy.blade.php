				...
				<div class="box-body">
					<form action="#" method="POST">
						<div class="form-group row">
							<div class="col-sm-10">
								<select name="ids[]" class="form-control selectpicker" id="lstFruits" multiple="multiple" data-live-search="true">
                  @foreach($tests as $test)
                  <option  name="megatest_id[{{$test->megatest_id}}]" value="megatest_id-{{$test->megatest_id}}" onchange='setmegaid({{$test->megatest_id}})' >{{$test->test_name}}</option>
                  @endforeach
                  @foreach($groups as $group)
                  <option name="grouptest_id[{{$group->grouptest_id}}]" value="grouptest_id-{{$group->grouptest_id}}" >{{$group->test_name}}</option>
                  @endforeach
                  @foreach($tests2 as $test2)
                  <option name="test_id[{{$test2->test_id}}]" value="test_id-{{$test2->test_id}}" >{{$test2->abbrev}}</option>
                  @endforeach
