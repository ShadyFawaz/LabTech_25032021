...
<title> LabTech Software (New Mega Test Child)</title>  
<style>   
Body {  
  font-family:  serif;  
...
@endif
<form role="form" onsubmit="return validateForm()" action = "{{url('newmegatestschildcreate/'.$megatest_id)}}" method = "post">
    {{ method_field('post') }}
    {{ csrf_field() }}
   
    <center> <h1  > {{__('title.create_new_megatestchild')}} </h1> </center>   
        <div >   
        <b><label>{{__('title.parent_name')}} </label>   </b>
            <input type="number" hidden  name="megatest_id" value="{{$megatest_id}}" required> 
            <b><label>{{$test_name}} </label>   </b>
            </div>
...
@endforeach
            ...
            <div>
            <button type="submit" class="NewMegaTestChild"> <b>{{__('title.new_megatestchild_btn')}}</button></b>    
