...
<title> LabTech Software (New Mega Test)</title>  
<style>   
Body {  
  font-family:  serif;  
...
@endif
<form name="NewMegaTest"  onsubmit="return validateForm()" action = "{{url('newmegatestscreate')}}" method = "post">
    {{ method_field('post') }}
    {{ csrf_field() }}

  
    <center> <h1> {{__('title.create_new_megatest')}} </h1> </center>     
        <div >   
        ...
        <b><label >{{__('title.report_name_megatest')}} </label>   </b>
            ...
            <div>
            <textarea style="font-weight:bold;font-size:13px;width:40%;line-height:20px;" name="test_comment" ></textarea>
            </div>
</div>
...
return false;
            ...
            <button type="submit" class="NewPatient"> <b>{{__('title.new_megatest_btn')}}</button></b>     
