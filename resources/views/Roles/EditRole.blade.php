...
<html>   
<head>    
<title> LabTech Software (Edit Role) </title>  
<style>   
Body {  
  font-family: serif;  
...
@endif
<form action = "{{url('editrole/'.$roles[0]->id)}}" method = "post">
    {{ method_field('post') }}
    {{ csrf_field() }}
   
    <center> <h1> {{__('title.edit_role')}} </h1> </center>        
            <div >
            <b><label style="color:red;">{{__('title.rolename_roles')}}  </label></b>   
            <input type="string"  name="role_name" value="{{$roles[0]->name}}" required>
            </div>
...
<tr>
        ...
        <tr>
        <td > {{ $screen->name }}</td> 
        <td><input  id="id-{{$screen->id}}"  type="checkbox" name="id[{{$screen->id}}]" value="{{ $screen->id}}"  {{ $roles[0]->hasPermissionTo($screen->id)? "checked" :""}}></td>
        </tr>
        @endforeach
        </tbody>
</table>
...
<tr>
        ...
        <tr>
        <td > {{ $right->name }}</td> 
        <td><input  id="id-{{$right->id}}"  type="checkbox" name="id[{{$right->id}}]" value="{{ $right->id}}"  {{ $roles[0]->hasPermissionTo($right->id)? "checked" :""}}></td>
        </tr>
        @endforeach
        </tbody>
</table>
