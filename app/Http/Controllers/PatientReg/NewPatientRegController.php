...
// if ($user->patient_id){
	if(NewPatientDataModel::where('Patient_ID', $user->patient_id)->exists()){
			...
			$insurancefactor   = $RelativePriceLists->insurance_factor;
			// dd($insurancefactor);
			$PatientLoad       = ($PriceListsTests->price * (1 - $insurancefactor)) * $RelativePriceLists->patient_load;
			$InsuranceLoad     = ($PriceListsTests->price * (1 - $insurancefactor)) * $RelativePriceLists->insurance_load;
			...
			if ($group_check){
				...
				$GroupTest_ID      = GroupTestsModel::where('grouptest_id',$id)->first();
				$LabToLabPrice     = OutLabTestsModel::where('grouptest_id',$id)->where('outlab_id',$GroupTest_ID->outlab_id)->first();
